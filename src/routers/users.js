...
router.post('/users/login', async (req,res)=>{
    try{
        ...
        const token = await user.generatetoken()  // generatetoken is not kyword its user defined
        res.send({user,token})
    }catch(e)
    {
         res.status(400).send()
...
router.patch('/users/me',auth, async (req,res)=>{
    ...
    try{
        ...
        await req.user.save()  // the above code is used to run the middleware (passsword hashing) 
